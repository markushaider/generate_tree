...
int64_t _find_biggest_parent(int64_t h_start) {
  int64_t i, max_i = h_start;
  for (i=h_start+1; i<num_halos; i++)
    if (halos[i].r > halos[max_i].r) max_i = i;
  return max_i;
}
...
void _fix_parents(int64_t h_start) {
  ...
  for (i=h_start; i<num_halos; i++) {
    if (extra_info[i].sub_of == i) {
      extra_info[i].sub_of = -1;
      continue;
    }

    int64_t next_child = extra_info[extra_info[i].sub_of].child;
    extra_info[i].next_cochild = next_child;
    extra_info[i].prev_cochild = -1;
    extra_info[extra_info[i].sub_of].child = i;
    if (next_child > -1)
...
void _find_subs(struct fof *f, int level) {
  ...
  max_i = _find_biggest_parent(h_start);
  ...
  if (num_growing_halos==1) {
	...
	particle_halos[p_start + j] = 
	  find_best_halo(copies+p_start+j, growing_halos[0]) - halos;
  }
...
void norm_sd(struct fof *f, float thresh) {
  ...
  for (j=0; j<6; j++) var[j] = pos2[j]/(double)f->num_p;
